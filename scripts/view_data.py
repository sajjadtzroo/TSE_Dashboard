...
with db.get_session() as session:
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.writer(f)
        writer.writerow([
            'InsCode', 'Symbol', 'Name', 'Date', 'Last Price',
            'First Price', 'Change %', 'Volume', 'Value', 'Trades'
        ])
...
with db.get_session() as session:
    funds = session.query(Company, FinancialIndicator).outerjoin(
...
with db.get_session() as session:
    high_vol = session.query(Company, DailyPrice).join(
        DailyPrice
    ).filter(
...
with db.get_session() as session:
    ...
    total_stocks = session.query(Company).filter(Company.ins_code < 2000000000000000).count()
    total_funds = session.query(Company).filter(Company.ins_code >= 2000000000000000).count()
    ...
    gainers_count = session.query(DailyPrice).filter(DailyPrice.price_change_percent > 0).count()
    losers_count = session.query(DailyPrice).filter(DailyPrice.price_change_percent < 0).count()
    unchanged_count = session.query(DailyPrice).filter(DailyPrice.price_change_percent == 0).count()
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        writer.writerow([''])
        writer.writerow(['Average Change %', f"{stats.avg_change:.2f}" if stats.avg_change else '0'])
...
print("\nExported files:")
print("  1. stocks_data.csv       - All {0} stocks".format(total_stocks))
print("  2. funds_data.csv        - All {0} funds".format(total_funds))
