...
with db.get_session() as session:
    ...
    for i, (company, price) in enumerate(top_volume, 1):
        print(
            f"{i:2d}. {company.symbol:15s} - Volume: {price.q_tot_tran_5j:>15,} - Last: {price.price_last}"
        )
...
with db.get_session() as session:
    ...
    for i, (company, price) in enumerate(gainers, 1):
        print(
            f"{i:2d}. {company.symbol:15s} - Change: {price.price_change_percent:>6.2f}% - Last: {price.price_last}"
        )
...
with db.get_session() as session:
    ...
    print(
        f"Total trading volume:     {stats.total_volume:,}"
        if stats.total_volume
        else "N/A"
    )
    print(
        f"Total trading value:      {stats.total_value:,}"
...
with db.get_session() as session:
    ...
    funds_with_nav = (
        ...
        .filter(FinancialIndicator.nav.isnot(None))
        .count()
    )

    avg_nav = (
        session.query(func.avg(FinancialIndicator.nav))
        .filter(FinancialIndicator.nav.isnot(None))
        .scalar()
    )
...
with db.get_session() as session:
    results = (
        session.query(Company, DailyPrice)
        .outerjoin(DailyPrice, Company.ins_code == DailyPrice.ins_code)
        .filter(Company.name_fa.like("%فولاد%"))
        .limit(5)
        .all()
    )
    ...
    for company, price in results:
        if price:
            print(
                f"- {company.symbol}: {company.name_fa} - Last: {price.price_last}, Change: {price.price_change_percent}%"
            )
        else:
