      ...
      <Table striped highlightOnHover withTableBorder={false} style={{ fontSize: '0.85rem' }}>
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th style={{ textAlign: 'right' }}>بازده</Table.Th>
          </Table.Tr>
        </Table.Thead>
        <Table.Tbody>
          {scenarios.map((s, i) => (
            <Table.Tr key={i} style={{ color: rowColors[i] || rallyColors.textPrimary }}>
              ...
              <Table.Td style={{ textAlign: 'right' }}>{formatNum(Math.round(s.targetPrice))}</Table.Td>
              <Table.Td style={{ textAlign: 'right', color: s.returnPct >= 0 ? rallyColors.green : rallyColors.red }}>
