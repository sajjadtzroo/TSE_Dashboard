...
class CMCKeyPool:
    """Thread-safe round-robin pool of CMC API keys."""

    def __init__(self):
        keys = []
        # Collect keys from CMC_API_KEYS (comma-separated) and CMC_API_KEY (single)
        if CMC_API_KEYS:
            keys.extend(k.strip() for k in CMC_API_KEYS.split(",") if k.strip())
        if CMC_API_KEY and CMC_API_KEY not in keys:
            keys.append(CMC_API_KEY)
        if not keys:
            raise ValueError("No CMC API keys configured (set CMC_API_KEY or CMC_API_KEYS)")
        self._keys = keys
        self._cycle = itertools.cycle(keys)
        self._lock = threading.Lock()
        logger.info(f"CMC key pool initialized with {len(keys)} key(s)")
...
class CMCClient:
    """Synchronous HTTP client for the CoinMarketCap Pro API with key rotation."""

    def __init__(self, key_pool: CMCKeyPool, base_url: str = CMC_BASE_URL, timeout: float = 15.0):
        self.base_url = base_url.rstrip("/")
        self._key_pool = key_pool
        self.timeout = timeout

    def _get(self, path: str, params: dict | None = None) -> dict:
        """Execute a GET request using the next API key in the pool."""
    ...
    def fetch_listings(self, limit: int = 200) -> list[dict]:
        """
...
_key_pool = CMCKeyPool()
_cmc = CMCClient(_key_pool)
