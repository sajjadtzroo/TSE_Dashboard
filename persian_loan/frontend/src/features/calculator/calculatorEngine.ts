...
export function analyzeLoan(
  ...
  try {
    ...
    // Try coefficient table first (any loan that has one)
    if (loan.coefficientTable && loan.coefficientTable.length > 0) {
      ...
      const matchingCoefficient = loan.coefficientTable.find(
        (row) => row.depositMonths === inputs.depositMonths ||
                 row.avgMonths === inputs.depositMonths ||
                 row.avgMonths === String(inputs.depositMonths)
      );

      if (matchingCoefficient) {
        const coefficientStr = matchingCoefficient.loanPercent || matchingCoefficient.coefficient || '100%';
        // Parse first number from strings like "80%", "230%", "80-230%"
        const match = coefficientStr.match(/(\d+)/);
        if (match) {
          depositMultiplier = parseFloat(match[1]) / 100;
          loanAmount = inputs.depositAmount * depositMultiplier;
        }
      }

      // If no exact match, find closest available month
      if (depositMultiplier === null) {
        let closestRow = null;
        let minDiff = Infinity;
        for (const row of loan.coefficientTable) {
          const rowMonths = typeof row.depositMonths === 'number' ? row.depositMonths :
                           typeof row.avgMonths === 'number' ? row.avgMonths :
                           parseInt(String(row.avgMonths || '0'));
          if (!isNaN(rowMonths) && rowMonths > 0) {
            const diff = Math.abs(rowMonths - inputs.depositMonths);
            if (diff < minDiff) {
              minDiff = diff;
              closestRow = row;
            }
        ...
        if (closestRow) {
          const coeffStr = closestRow.loanPercent || closestRow.coefficient || '100%';
    ...
    // Fallback: use loanMultiplier field (e.g., "370%", "200%")
    ...
    // Fallback: use depositToFacilityRatio (e.g., "1:3.7", "1:2")
    ...
    // Fallback: for loans without any deposit info, use max amount
    if (depositMultiplier === null) {
...
export function analyzeLoanWithAdvancedMetrics(
  ...
  try {
    ...
    // Use deposit as equity proxy when loan has deposit-related data
    const hasDepositData = loan.coefficientTable?.length ||
      loan.loanMultiplier || loan.depositToFacilityRatio ||
      loan.category?.includes('deposit');
