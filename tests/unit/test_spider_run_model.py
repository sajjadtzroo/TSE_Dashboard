...
class TestSpiderRunModel:
    ...
    def test_multiple_spider_runs(self, db_session):
        ...
        market_runs = db_session.query(SpiderRun).filter_by(
            spider_name='market_watch'
        ).all()
    ...
    def test_spider_run_with_metadata(self, db_session):
        ...
        retrieved = db_session.query(SpiderRun).filter_by(
            spider_name='history_backfill'
        ).first()
    ...
    def test_query_by_status(self, db_session):
        ...
        runs = [
            SpiderRun(spider_name='spider1', start_time=now, status='success', items_scraped=10),
            SpiderRun(spider_name='spider2', start_time=now, status='failed', items_scraped=5, error_count=2),
            SpiderRun(spider_name='spider3', start_time=now, status='success', items_scraped=20),
            SpiderRun(spider_name='spider4', start_time=now, status='running', items_scraped=0),
        ]
    ...
    def test_query_by_time_range(self, db_session):
        ...
        recent_runs = db_session.query(SpiderRun).filter(
            SpiderRun.start_time >= cutoff_naive
        ).all()
...
class TestSpiderRunQueries:
    ...
    def test_get_latest_run_per_spider(self, db_session):
        ...
        runs = [
            SpiderRun(spider_name='spider_a', start_time=now - timedelta(hours=2), status='success', items_scraped=10),
            SpiderRun(spider_name='spider_a', start_time=now - timedelta(hours=1), status='success', items_scraped=15),
            SpiderRun(spider_name='spider_a', start_time=now, status='success', items_scraped=20),
            SpiderRun(spider_name='spider_b', start_time=now - timedelta(hours=1), status='failed', items_scraped=5),
            SpiderRun(spider_name='spider_b', start_time=now, status='success', items_scraped=10),
        ]
        ...
        latest_a = db_session.query(SpiderRun).filter_by(
            spider_name='spider_a'
        ).order_by(SpiderRun.start_time.desc()).first()

        assert latest_a.items_scraped == 20
        # SQLite doesn't preserve timezone info, so compare times without timezone
        assert latest_a.start_time.replace(tzinfo=timezone.utc) >= now - timedelta(seconds=1)
    ...
    def test_calculate_success_rate(self, db_session):
        ...
        runs = [
            SpiderRun(spider_name='spider', start_time=now - timedelta(hours=5), status='success', items_scraped=10),
            SpiderRun(spider_name='spider', start_time=now - timedelta(hours=4), status='success', items_scraped=10),
            SpiderRun(spider_name='spider', start_time=now - timedelta(hours=3), status='failed', items_scraped=0),
            SpiderRun(spider_name='spider', start_time=now - timedelta(hours=2), status='success', items_scraped=10),
            SpiderRun(spider_name='spider', start_time=now - timedelta(hours=1), status='failed', items_scraped=0),
        ]
