...
| Cache key | MD5 hash of context (first 16 chars) |
...
| Reranker | Disabled (MiniLM-6-v2 available) |
...
| Cache key | MD5 hash (first 16 chars) |
...
| Search results | Full search response | 5-60 min (dynamic) | Redis |
| Agent instances | Agent configuration | Infinite (in-process) | functools.cache |

**Dynamic TTL Strategy**: Search cache uses shorter TTL during trading hours (300s) and longer off-hours (3600s), matching the market data cache behavior.
...
| Search quality | 8/10 | Hybrid BM25 + pgvector with RRF; bilingual support |
| Caching strategy | 8/10 | Multi-layer caching (router, embedding, search, agent) |
...
### Overall RAG Grade: **B+ (8.0/10)**

The RAG system is well-architected with clean separation of concerns (router → agent → tools). The hybrid search with RRF fusion provides quality results. Main limitations are external LLM API latency and single-point-of-failure on the OpenRouter API key.

---

## Recommendations

1. **Enable reranker**: The MiniLM-6-v2 cross-encoder is configured but disabled — enable for improved search quality
2. **Add request queuing**: Implement semaphore or queue for concurrent LLM API calls to prevent rate limiting
3. **Tool result caching**: Cache frequent tool results (market indices, sector data) to reduce DB round-trips during chat
4. **Streaming for tool rounds**: Currently only the final answer is streamed — consider streaming intermediate results
5. **Observability**: Add Prometheus metrics for router latency, tool execution time, and cache hit rates per agent type
