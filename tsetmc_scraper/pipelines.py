...
class DataCleaningPipeline:
    ...
    def _clean_order_book(self, adapter):
        ...
        for level in range(1, 6):
            for prefix in ["bid_price", "ask_price"]:
                adapter[f"{prefix}_{level}"] = safe_float(
                    adapter.get(f"{prefix}_{level}")
                )
            for prefix in ["bid_vol", "ask_vol"]:
                adapter[f"{prefix}_{level}"] = safe_int(
                    adapter.get(f"{prefix}_{level}")
                )
    ...
    def _clean_ime_option(self, adapter):
        ...
        adapter["contract_description"] = clean_text(
            adapter.get("contract_description")
        )
    ...
    def _clean_financial_statement(self, adapter):
        ...
        adapter["codal_announcement_id"] = safe_int(
            adapter.get("codal_announcement_id")
        )
...
class DatabasePipeline:
    ...
    def _load_security_cache(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded security cache: {len(self._sec_cache)} TSE + {len(self._market_sec_cache)} market entries"
            )
    ...
    def _resolve_security_id(self, ins_code):
        ...
        row = (
            ...
            .filter(Security.ins_code == ins_code)
            .one_or_none()
        )
        if row:
    ...
    def close_spider(self, spider):
        try:
            ...
            logger.info(
                f"Database pipeline stats - Processed: {self.items_processed}, "
    ...
    def _flush_companies(self, buffer):
        ...
        new_rows = (
            self.session.query(Security.ins_code, Security.security_id)
            .filter(Security.ins_code.in_(ins_codes))
            .all()
        )
    ...
    def _flush_order_books(self, buffer):
        ...
        stmt = (
            insert(OrderBook.__table__)
            .values(rows)
            .on_conflict_do_nothing(constraint="uq_order_book_sec_time")
        )
        self.session.execute(stmt)
    ...
    def _flush_codal(self, buffer):
        ...
        for item in buffer:
            ...
            if symbol:
                row = (
                    self.session.query(Security.security_id)
                    .filter(Security.symbol == symbol, Security.market_type == "tse")
                    .first()
                )
                if row:
                    sec_id = row[0]
    ...
    def _flush_financial_statements(self, buffer):
        ...
        update_cols = {
            ...
            if c.name
            not in ("id", "codal_announcement_id", "statement_type", "created_at")
        }
        stmt = stmt.on_conflict_do_update(
    ...
    def _flush_tick_trades(self, buffer):
        ...
        stmt = (
            insert(TickTrade.__table__)
            .values(rows)
            .on_conflict_do_nothing(constraint="uq_tick_trades_sec_date_row")
        )
        self.session.execute(stmt)
