...
from tsetmc_scraper.utils import BROWSER_UA, persian_to_english_numbers, safe_int
...
class CodalFinancialsDetailSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        announcements = self.session.query(CodalAnnouncement).filter(
        ...
        ).order_by(CodalAnnouncement.id).limit(self.batch_size).all()
    ...
    def parse_excel(self, response, announcement_id, letter_serial, symbol, company_name, title):
        ...
        storage_path = self._store_raw_html(letter_serial, html_body, announcement_id)
    ...
    def _parse_financial_tables(self, html_body, announcement_id, symbol, company_name, title):
        ...
        for statement_type, (line_items, hot_fields) in best_by_type.items():
            ...
            # Set hot fields
            for field_name in ['revenue', 'cost_of_revenue', 'gross_profit',
                               'operating_income', 'net_income', 'total_assets',
                               'total_liabilities', 'total_equity', 'eps']:
