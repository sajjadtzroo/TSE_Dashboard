    ...
    def parse(self, response):
        ...
        for rec in data:
            try:
                ...
                item['contract_size'] = self._int(rec.get('contract_size'))
                item['contract_size_unit'] = rec.get('contract_size_unit')
                item['date_end'] = rec.get('date_end')
                item['day_remain'] = self._int(rec.get('day_remain'))
                item['margin_initial'] = self._int(rec.get('margin_initial'))
                item['margin_maintenance'] = self._int(rec.get('margin_maintenance'))
                item['interest_open'] = self._int(rec.get('interest_open'))
                item['interest_open_change'] = self._int(rec.get('interest_openc'))
                item['interest_open_change_pct'] = self._num(rec.get('interest_openp'))
                ...
                item['instant_settlement'] = self._num(rec.get('pls'))
                ...
                # 3-level order book
                item['bid_price_1'] = self._int(rec.get('pd1'))
                item['bid_vol_1'] = self._int(rec.get('qd1'))
                item['ask_price_1'] = self._int(rec.get('po1'))
                item['ask_vol_1'] = self._int(rec.get('qo1'))
                item['bid_price_2'] = self._int(rec.get('pd2'))
                item['bid_vol_2'] = self._int(rec.get('qd2'))
                item['ask_price_2'] = self._int(rec.get('po2'))
                item['ask_vol_2'] = self._int(rec.get('qo2'))
                item['bid_price_3'] = self._int(rec.get('pd3'))
                item['bid_vol_3'] = self._int(rec.get('qd3'))
                item['ask_price_3'] = self._int(rec.get('po3'))
                item['ask_vol_3'] = self._int(rec.get('qo3'))
